function processData(games) {
    return games.map(game => {
        ...
        const subsets = dataParts.join('').split('; ').map(subset => {
            return subset.split(', ').reduce((acc, curr) => {
                const [quantity, color] = curr.split(' ');
                acc[color] = (acc[color] || 0) + parseInt(quantity);
                return acc;
            }, {});
        });
        return { id, subsets };
...
function sumData(data){
    const sumByColor = data.reduce((acc, item) => {
        if (acc[item.color]) {
          acc[item.color] += parseInt(item.quantity, 10);
        } else {
          acc[item.color] = parseInt(item.quantity, 10);
        }
        return acc;
...
Game 100: 6 blue, 10 green; 3 green, 4 blue, 1 red; 7 blue, 1 red, 12 green`.split('\n')

const processedData = processData(input);
const validGames = validate(processedData);
const sumOfIds = validGames.reduce((a, b) => a + b, 0);
